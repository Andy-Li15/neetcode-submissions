...
then skip left, decrement counter (and increment diff if necessary)
*/
class Solution {
    public String minWindow(String s, String t) {
        ...
        while (right < s.length()) {
            ...
            if (diff == 0) {
                ...
                // skip left chars until diff != 0 (char in t skipped)
                ...
                // skip left chars that are not in t
                while (left <= right && !freq1.containsKey(s.charAt(left))) {
                    left++;
                }
            } 
            right++;
